...
import IGratuityFormModel from "@/models/forms/gratuity-form";
...
export async function POST(req: NextRequest) {
    ...
    try {
        ...
        const body: IGratuityForm = await req.json();
        ...
        const existingForm = await IGratuityFormModel.findOne({ userId: actualUserId });

        let result;
        if (existingForm) {
            await IGratuityFormModel.updateOne({ userId: actualUserId }, { $set: body });
            result = await IGratuityFormModel.findOne({ userId: actualUserId });
        } else {
